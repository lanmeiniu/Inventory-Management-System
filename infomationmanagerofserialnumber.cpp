...
void InfomationManagerOfserialNumber::getAllDataFromMysql()
{
    QTime time;
    time.start();
    int columnCount = 0;
    ...
    QString selectQuery = QString("SELECT * FROM %1.%2").arg(databaseName).arg(tableName);
    qDebug() << selectQuery;
    columnCount = queryMysql(selectQuery);
...
//void InfomationManagerOfserialNumber::getAllDatasql()
//{
//    QString getSql;

//    getSql = QString("select * from %1").arg(tableName);
